...
class _CitizenLoginScreenState extends State<CitizenLoginScreen> {
  ...
  void _handleLogin() {
    ...
    if (_usuarioController.text.isEmpty) {
      setState(() => _usuarioError = 'Você precisa digitar o login');
      hasError = true;
    }
    ...
    if (!hasError) {
      AppData.userCpf = _usuarioController.text;
      AppData.userName = 'João Silva';
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const CitizenDashboardScreen()));
    }
  }
            ...
            child: Padding(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Container(width: 80, height: 80, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: const Icon(Icons.person, size: 50, color: Color(0xFF0066CC))),
                    const SizedBox(height: 20),
                    const Text('Acesso Usuário', style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    const Text('Portal Cidadão', style: TextStyle(color: Colors.white, fontSize: 18)),
                    const SizedBox(height: 32),
                    
                    _buildTextField(controller: _usuarioController, label: 'Usuário', hint: '000.000.000-00', error: _usuarioError, inputFormatters: [CpfInputFormatter()], keyboardType: TextInputType.number),
